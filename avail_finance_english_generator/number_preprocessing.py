...
hundreds = {}
singles = {}
for item in os.listdir(CURRENT_PATH+"/motilal_responses/THOUSAND/ENGLISH"):
    file_name = CURRENT_PATH+"/motilal_responses/THOUSAND/ENGLISH/" + item
    thousands[item] = AudioSegment.from_wav(file_name)

for item in os.listdir(CURRENT_PATH+"/motilal_responses/HUNDREDS/ENGLISH"):
    file_name = CURRENT_PATH+"/motilal_responses/HUNDREDS/ENGLISH/" + item
    hundreds[item] = AudioSegment.from_wav(file_name)
# print(thousands)

for item in os.listdir(CURRENT_PATH+"/motilal_responses/NUMBERS/ENGLISH"):
    file_name = CURRENT_PATH+"/motilal_responses/NUMBERS/ENGLISH/" + item
...
def get_recorded_voice_for_number(number):
    raw_number = number
    print("numer",number)
    number = num2words(number,lang="en_IN")
    ...
    for z in number:
        ...
        if "thous" in z:
            ...
            new_number=z.replace("lakh","").strip()
            print("New number:",new_number)
            x=str(text2int(new_number))
            if str(x)+".wav" in singles:
                if final is None:
                    final=singles[x+".wav"]+AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/lakh_english.wav")
                else:
                    final+=singles[x+".wav"]+AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/lakh_english.wav")
            else:
                raise Exception("Number is missing ",x)
...
print(get_recorded_voice_for_number(number = 100))