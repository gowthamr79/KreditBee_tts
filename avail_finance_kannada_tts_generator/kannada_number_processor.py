...
def get_recorded_voice_for_number(number):
    ...
    if number == 9000:
        print("fsaf")
    number = num_to_word(number, lang='kn')
    print(number, raw_number)
    word_2_number = number.split(",")
    # print("32343t54y6uioiuytrew", number, word_2_number)
    word_2_number_dict = dict([(value, key) for key, value in num_dict['kn'].items()])
    final = None
    for item in word_2_number:
        if "ಸಾವಿರದ" in item or "ಸಾವಿರ" in item:
            item = item.replace("ಸಾವಿರದ", "")
            item = item.replace("ಸಾವಿರ", "")
            item = item.strip()
            if item in word_2_number_dict:
                if final is None:
                    final = AudioSegment.from_wav(CURRENT_PATH+"/avail_finance/KANNADA THOUSANDS/{}.wav".format(int(word_2_number_dict[item]) * 1000))
                else:
                    final += AudioSegment.from_wav(CURRENT_PATH+"/avail_finance/KANNADA THOUSANDS/{}.wav".format(int(word_2_number_dict[item]) * 1000))
            else:
                raise ValueError("number not found {} and text is {}".format(number, raw_number))
        elif "ನೂರ" in item or "ನೂರು" in item:
            item = item.split(" ")
            if "ನೂರ" in item:
                item.remove("ನೂರ")
            if "ನೂರು" in item:
            ...
            if item in word_2_number_dict:
                if final is None:
                    final = AudioSegment.from_wav(CURRENT_PATH+"/avail_finance/KANNADA HUNDREDS/{}.wav".format(int(word_2_number_dict[item]) * 100))
                else:
                    final += AudioSegment.from_wav(CURRENT_PATH+"/avail_finance/KANNADA HUNDREDS/{}.wav".format(int(word_2_number_dict[item]) * 100))
            else:
                raise ValueError("number not found {} and text is {}".format(item, raw_number))
        elif item.strip() in word_2_number_dict:
            item = item.strip()
            if final is None:
                final = AudioSegment.from_wav(CURRENT_PATH+"/avail_finance/KANNADA NUMBERS/{}.wav".format(word_2_number_dict[item]))
            else:
                final += AudioSegment.from_wav(CURRENT_PATH+"/avail_finance/KANNADA NUMBERS/{}.wav".format(word_2_number_dict[item]))
        else:
            raise ValueError("number not found {} and text is {}".format(number, raw_number))
    if final:
        final.export(CURRENT_PATH+"/kannada_numbers/{}.wav".format(raw_number), format="wav")
    else:
        raise ValueError("number not found {} and text is {}".format(number, raw_number))
    return final

# print(get_recorded_voice_for_number(500))
