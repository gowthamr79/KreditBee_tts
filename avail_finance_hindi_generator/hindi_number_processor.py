...
def get_recorded_voice_for_number(number):
    raw_number = number
    number = num_to_word(number, lang='hi')
    word_2_number = number.split(",")
    # print("32343t54y6uioiuytrew", number, word_2_number)
    word_2_number_dict = dict([(value, key) for key, value in num_dict['hi'].items()])
    final = None
    for item in word_2_number:
        if "हज़ार" in item:
            item = item.replace("हज़ार", "")
            item = item.strip()
            if item in word_2_number_dict:
                if final is None:
                    final = AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/THOUSAND/HINDI/{}.wav".format(int(word_2_number_dict[item]) * 1000))
                else:
                    final += AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/THOUSAND/HINDI/{}.wav".format(int(word_2_number_dict[item]) * 1000))
        elif "सौ" in item:
            item = item.replace("सौ", "")
            item = item.strip()
            if item in word_2_number_dict:
                if final is None:
                    final = AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/HUNDREDS/HINDI/{}.wav".format(int(word_2_number_dict[item]) * 100))
                else:
                    final += AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/HUNDREDS/HINDI/{}.wav".format(int(word_2_number_dict[item]) * 100))
        elif "लाख" in item:
            item=item.replace("लाख","")
            item=item.strip()
            if item in word_2_number_dict:
                if final is None:
                    final=AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/NUMBERS/HINDI/{}.wav".format(int(word_2_number_dict[item])))+AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/lakh_hindi.wav")
                else:
                    final+=AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/NUMBERS/HINDI/{}.wav".format(int(word_2_number_dict[item])))+AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/lakh_hindi.wav")
        elif item.strip() in word_2_number_dict:
            item = item.strip()
            if final is None:
                final = AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/NUMBERS/HINDI/{}.wav".format(word_2_number_dict[item]))
            else:
                final += AudioSegment.from_wav(CURRENT_PATH+"/motilal_responses/NUMBERS/HINDI/{}.wav".format(word_2_number_dict[item]))
    # if final:
    #     final.export("formatted_numbers/{}.wav".format(raw_number), format="wav")
    return final
